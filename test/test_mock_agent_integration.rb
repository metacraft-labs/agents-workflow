...
class TestMockAgentIsolation < Minitest::Test
  ...
  def test_single_agent_workspace_creation
    ...
    assert generated_files.size > 0, "Agent should create some files"
  ...
  def test_concurrent_agents_isolation
    ...
    results.each_with_index do |result, i|
      ...
      assert result[:activity_count] > 0, "Agent #{i + 1} should perform activities"
    end
    ...
    workspaces.each_with_index do |workspace, i|
      ...
      assert agent_files.size > 0, "Agent #{i + 1} should generate files"

      # Check that no other agent's files exist in this workspace
      other_agent_files = workspaces.each_with_index.flat_map do |other_workspace, j|
        next [] if i == j
  ...
  def test_performance_characteristics
    ...
    when Snapshot::CopyProvider
      ...
      assert avg_creation_time < 10.0, "Copy provider should complete within reasonable time (avg: #{avg_creation_time}s)"
    end
  end
  ...
  def test_filesystem_modification_isolation
    ...
    shared_files.each do |file|
      ...
      if checksum1 != initial_checksums[file] || checksum2 != initial_checksums[file]
  ...
  def test_large_scale_concurrent_execution
    ...
    (1..num_agents).each do |i|
      workspaces << create_isolated_workspace("stress_#{i}")
    end
